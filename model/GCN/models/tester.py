...
from model.GCN.utils.math_utils import evaluation
...
def multi_pred(sess, y_pred, seq, batch_size, n_his, n_pred, step_idx, dynamic_batch=True ):
    ...
    for i in gen_batch(seq, min(batch_size, len(seq)), dynamic_batch=dynamic_batch):
        ...
        gat_test_seq = np.copy(i[:, 0:n_his + 1, :, :])
        n_node = gat_test_seq.shape[2]
        step_list = []
        for j in range(n_pred):
            pred = sess.run(y_pred,
                            feed_dict={'data_input:0': test_seq[:,:,:,2:3], 'keep_prob:0': 1.0,'Gat_data_input:0':gat_test_seq.reshape((-1,n_node,3))})
...
def model_inference(sess, pred, inputs, batch_size, n_his, n_pred, step_idx, min_va_val, min_val):
    ...
    # traffic_data_val = traffic_data['val']
    # traffic_data_test = traffic_data['test']
    ...
    # print(y_val.shape)
    evl_val = evaluation(x_val[0:len_val, step_idx + n_his, :, 2:3], y_val, x_stats)
    ...
    if sum(chks):
        ...
        evl_pred = evaluation(x_test[0:len_pred, step_idx + n_his, :, 2:3], y_pred, x_stats)
        min_val = evl_pred
...
def model_test(inputs,batch_size, n_his, n_pred, inf_mode, load_path='./output/models/'):
    ...
    with tf.Session(graph=test_graph) as test_sess:
        ...
        evl = evaluation(x_test[0:len_test, step_idx + n_his, :, 2:3], y_test, x_stats)
